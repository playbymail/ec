...
| (resources/js/hooks/use-appearance.tsx) reads back the same cookie so hydration re-applies what is
...
test('a persisted appearance choice is what the next full page load paints', function (string $choice, bool $expectsDarkClass) {
    /*
     * There is no server-side writer for this cookie: resources/js/hooks/use-appearance.tsx writes
     * it from updateAppearance() when the user picks a theme, as a plain unencrypted cookie.
     * Sending exactly what that function writes covers the server-read half of the round trip.
     */
...
test('the client theme store reads the cookie and keeps no second source of truth', function () {
     ...
     * held in place. This read localStorage first, which is a store the server cannot see: whenever
     * it drifted from the cookie, hydration flipped the theme away from what had already been
     * painted. Reintroducing a read or a write here would restore that flash silently, so assert
     * the cookie is the only store. Matching on the call sites, not the word, leaves the
     * explanatory comment in that file free to name what was removed.
     */
    $store = file_get_contents(resource_path('js/hooks/use-appearance.tsx'));
...
test('the resolving script and the html class agree on an explicit choice', function () {
    /*
     * The two mechanisms read the same cookie, so they must never contradict each other: the class
     * is what paints, and the script is what runs a moment later. A dark cookie has to produce both.
     */
