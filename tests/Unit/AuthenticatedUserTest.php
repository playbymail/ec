...
|--------------------------------------------------------------------------
|
| Every caller sits behind the `auth` middleware, so the null branch of this guard is
| unreachable over HTTP — which is exactly why it needs a test of its own. Nothing in the
| feature suite would notice if the guard stopped guarding, and the point of it is that
| PHPStan level 8 is satisfied by real behaviour rather than by an assertion about behaviour.
|
*/
